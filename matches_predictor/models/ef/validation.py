...
from matches_predictor.models.ef import test_set, train_set, prediction
import random
import itertools
...
def _get_accuracy(df, threshold=0.33):
    df = df.loc[(df['probability_final'] > threshold) | (df['probability_final'] < (1-threshold)), :]
    predictions_final = df['prediction_final_encoded']
    true_y = df['final_ef']
    acc = accuracy_score(true_y, predictions_final)
    return acc
...
def _show_df(input_df):
    final_df = input_df.loc[:, ['minute', 'home_score',
                                'away_score', 'probability_final', 'prediction_final_encoded', 'final_ef']]
    return final_df
...
def save_model(best_estimator, acc, force_saving):
    ...
    with open(f"{path}/ef_accuracy.txt", 'r') as f:
